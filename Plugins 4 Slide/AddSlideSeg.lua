...
function Seg_Delete(seg_table, index_table, cur_beat, slide)
    if (#index_table) == 1 then --当前所处beat在某个节点上
        Editor:ShowMessage("此处已存在节点")
        return
    else --当前所处beat在两个节点之间
