...
function easing(x,steps,i)
  ...
  --local percentage=1-(1-xx)*(1-xx)
  --local percentage=2^(-10 * xx) * math.sin((xx * 10 - 0.75) * Elastic_count) + 1
  --local percentage=1 + BackCount2 * (xx - 1) ^ 3 + BackCount1 * (xx - 1) ^ 2
  local percentage=1-(1-xx)^3
  --local percentage=1-2^(-10*xx)
